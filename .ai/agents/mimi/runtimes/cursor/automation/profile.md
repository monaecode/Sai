# Cursor automation profile — Mimi (mimi) — SECONDARY runtime

> Mimi's primary runtime is **claude-code-cli**; the live automation truth
> lives in `../../claude/automation/profile.md` and `hooks.json`. This
> Cursor Automations UI walkthrough is the **optional secondary** runtime
> path for monaecode — no Cursor automation for Mimi exists as of
> 2026-07-17. The capability dates below (2026-07-14) reflect the original
> initialization survey; re-verify in a Cursor session before relying on
> them.
>
> Generated 2026-07-17 by `scripts/agent-automation-spec` for principal
> **monaecode (U0BGNS7F0T1)**; relocated from `runtimes/claude/automation/`
> per Saul CTO review Dezocode/Sai#27 (Cursor-owned state does not belong
