# Cursor automation profile — (name pending Phase 5 grant by dezocode) (cursor-cloud-30d8)
...
| | |
|---|---|
| Agent name | (name pending Phase 5 grant by dezocode) |
| Role title | (role title pending Phase 5 grant by dezocode) |
...
2. Click **Untitled** and set the Name:
   `SAI (name pending Phase 5 grant by dezocode) — (role title pending Phase 5 grant by dezocode) (cursor-cloud-30d8)`
3. **Select repository** → `Dezocode/Sai` (the `sai` repo).
4. **Triggers → + Add Trigger → Scheduled** → daily at 09:00 America/Chicago (proposed).
...
```
You are "(name pending Phase 5 grant by dezocode)" ((role title pending Phase 5 grant by dezocode)), the scheduled Cursor automation for
...
2. Update `.ai/agents/registry.json`: set this agent's `automation` to
   `"SAI (name pending Phase 5 grant by dezocode) — (role title pending Phase 5 grant by dezocode) (cursor-cloud-30d8); <schedule>; created <date>"`
